...
class BaseEditor:
    ...
    @classmethod
    def from_hparams(cls, hparams: HyperParams, model=None, tok=None):
        return cls(hparams, model, tok)

    def __init__(self, hparams: HyperParams, model=None, tok=None):
    ...
    def create_model(self, ):
        if type(self.model_name) is str:
            ...
            if hasattr(self.hparams, 'fp16') and self.hparams.fp16:
                torch_dtype = torch.float16
            elif hasattr(self.hparams, 'bfp16') and self.hparams.bfp16: 
            ...
            if 't5' in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name,trust_remote_code=True, torch_dtype=torch_dtype if self.hparams.alg_name not in ['MEND'] else torch.bfloat16, device_map=device_map)
            ...
            if self.tok is not None and (isinstance(self.tok, GPT2Tokenizer) or isinstance(self.tok, GPT2TokenizerFast) or isinstance(self.tok, LlamaTokenizer)) and (self.hparams.alg_name not in ['ROME', 'MEMIT', 'EMMET', 'R-ROME']):
                LOG.info('AutoRegressive Model detected, set the padding side of Tokenizer to left...')
                self.tok.padding_side = 'left'
            if self.tok is not None and ('mistral' in self.model_name.lower() or 'llama' in self.model_name.lower() or 'qwen' in self.model_name.lower()) and (self.hparams.alg_name in ['ROME', 'MEMIT', 'EMMET', 'R-ROME']):
        ...
        if not self.hparams.model_parallel and hasattr(self.hparams, 'device'):
    ...
    def batch_edit(self,
        ...
        requests = _prepare_requests(prompts, target_new, ground_truth, rephrase_prompts,locality_prompts, locality_ground_truth, **kwargs)
        ...
        for record_chunks in _chunks(requests, self.hparams.batch_size):
            ...
            edited_model, weights_copy, *_ = self.apply_algo(
                self.model,
                self.tok,
                record_chunks,
                self.hparams,
        ...
        return all_metrics, edited_model, weights_copy, _
    ...
    def edit_requests(self,
        ...
        def edit_func(request):
            if self.alg_name == 'IKE':
                ...
                icl_examples = None
            return edited_model, weights_copy, icl_examples, _
        ...
        if sequential_edit:
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                edited_model, weights_copy, icl_examples, loss = edit_func(request)
        ...
        return all_metrics, edited_model, weights_copy, loss
